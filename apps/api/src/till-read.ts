...
export async function getPolicy(owner: string, tokenId: string) {
  ...
  return {
    ...
    allowedServices: ['Safety', 'Market', 'Contract'],
    allowedAssets: ['USDC.e (x402)', '0G (Till balance)'],
    ...
    note: '0G vault caps are native 0G. The $0.50 mission cap is the USDC.e application cap for Before You Pay. Service categories are selected by the agent from live x402 quotes, not stored as an on-chain enum.',
  }
}
...
export async function getSession(owner: string, tokenId: string) {
  ...
  return {
    ...
    note: 'The session private key never leaves the owner device. MCP sees only the authorized address.',
